...
def cbv(router: Blueprint) -> Callable[[Type[T]], Type[T]]:
    """
...
def _cbv(router: Blueprint, cls: Type[T]) -> Type[T]:
    """
    ...
    _init_cbv(cls)
    cbv_router = Blueprint(router.name, router.import_name)
    function_members = inspect.getmembers(cls, inspect.isfunction)
    functions_set = set(func for _, func in function_members)
    print(functions_set)
    print(router._blueprints)
    cbv_routes = [
        route
        for route in router._blueprints
        if isinstance(route, (Blueprint)) and route.endpoint in functions_set
    ]
    for route in cbv_routes:
       # router._blueprints.remove(route)
       # _update_cbv_route_endpoint_signature(cls, route)
        print(route)
        cbv_router._blueprints.append(route)
